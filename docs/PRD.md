...
- `decayStyle(n, rates?)`: `scale = max(0.04, rates.scale ** n)`, `opacity = max(0.03, rates.opacity ** n)`. Tasas por defecto (desktop, `DESKTOP_DECAY_RATES`): `{ scale: 0.985, opacity: 0.992 }` — decae **muy despacio** a propósito (a los 20 intentos sigue casi entero, mitad de tamaño hacia los ~50) y sin piso real: con persistencia suficiente (~200+) el botón se vuelve imperceptible, nunca se congela ni se reinicia. En móvil (`useIsMobileDevice` en `composables/useEvasiveTarget.ts`) se usan tasas más agresivas, `{ scale: 0.75, opacity: 0.85 }`, para que la rendición llegue en como máximo 5 toques.
...
El cambio de texto (label del botón, nota de rendición) **no** dispara a un número fijo de intentos: espera a que el encogimiento progresivo (`decayStyle`) vuelva al botón visiblemente pequeño (`scale` cae más rápido que `opacity`, así que en la práctica es el que dispara el umbral). En desktop (tasa 0.985) eso ronda los ~80 intentos; en móvil (tasa 0.75, ver arriba) el umbral se cruza a más tardar en el 5º intento. `useIsMobileDevice` decide desktop-vs-móvil **una sola vez, al cargar la página** (evalúa `matchMedia` al importar el módulo, no reacciona a resize/rotación), así que una instancia del modal nunca cambia de "personalidad" a mitad de partida. El componente padre (`DeclineModal`) escucha `surrender` y muestra la línea de rendición. El modal **no** se cierra solo. El estado (intentos, rendición) persiste **mientras el modal esté abierto** — sobrevive a intentos repetidos de pulsar el botón — pero se reinicia por completo al cerrar el modal (Escape o "mejor sí acepto"; el click en el backdrop **no** cierra el modal): `DeclineModal` fuerza el remount de `EvasiveButton` vía `:key`, así que la próxima apertura siempre arranca en `attempts = 0`, sin rendición previa.
